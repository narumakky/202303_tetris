...
#            self.model.cuda()
    ...
    def step_v2(self, curr_backboard, action, curr_shape_class):
        ...
        reward_bamp = -self.reward_weight[0] * bampiness
        # 最大高さ罰
        if max_height > self.max_height_relax:
            reward -= self.reward_weight[1] * max(0, max_height)

        reward_height = -self.reward_weight[1] * max(0, max_height)
        # 穴の数罰
        reward -= self.reward_weight[2] * hole_num
        reward_ana = -self.reward_weight[2] * hole_num
        # 穴の上のブロック数罰
        reward -= self.hole_top_limit_reward * hole_top_penalty * max_highest_hole
        reward_ana_b = -self.hole_top_limit_reward * hole_top_penalty * max_highest_hole
        # 左端以外埋めている状態報酬
        reward += tetris_reward * self.tetris_fill_reward
        reward_hidariigai = tetris_reward * self.tetris_fill_reward
        # 左端が高すぎる場合の罰
        ...
        reward_hidartakai = - (left_side_height - self.bumpiness_left_side_relax) * \
                self.left_side_height_penalty

        self.epoch_reward += reward

        if lines_cleared >= 1:
            log = "ttl: {:.4f} Clines: {} l: {:.4f} b {:.4f} ht {:.4f} ana {:.4f} ana_b {:.4f} higai {:.4f} htaka {:.4f}".format(
                self.epoch_reward,
                lines_cleared,
                reward_line,
                reward_bamp,
        ...
        if self.mode == "train" or self.mode == "train_sample" or self.mode == "train_sample2":
            ...
            if self.predict_next_num_train > 0:
                ...
                index_list, index_list_to_q, next_actions, next_states \
                    = self.get_predictions(self.model, False, GameStatus, next_steps, self.predict_next_steps_train, 1, self.predict_next_num_train, index_list, index_list_to_q, -60000)
...
#                 next2_state = next2_states[next_index, :]
        ...
        ###############################################
            ...
            if self.predict_next_num > 0:
                ...
                ## 最大値 の index 取得
                index = torch.argmax(predictions).item()
                hold_index = torch.argmax(predictions).item()
